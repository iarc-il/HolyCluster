...
function connect_to_radio() {
	...
	const websocket_url =
		(protocol == "https:" ? "wss:" : "ws:") + "//" + host + "/radio";
...
function MainContainer() {
	...
	let show_only_filters = callsign_filters.filters.filter(
		(filter) => filter.action == "show_only"
	);
	let hide_filters = callsign_filters.filters.filter(
		(filter) => filter.action == "hide"
	);
	let alerts = callsign_filters.filters.filter(
	...
	const [table_sort, set_table_sort] = use_object_local_storage(
		"table_sort",
		{
			column: "time",
			ascending: false,
	...
	useEffect(() => {
		const fetch_spots_with_context = fetch_spots.bind(
			fetch_spots_context.current
		);
		fetch_spots_with_context();
		let spots_interval_id = setInterval(
			fetch_spots_with_context,
			30 * 1000
		);
		...
		let propagation_interval_id = setInterval(
			fetch_propagation_with_context,
			3600 * 1000
		);
    ...
    const filtered_spots = spots
        .filter(spot => {
			...
			const is_dx_continent_active =
				filters.dx_continents[spot.dx_continent];
	...
	let { send_message_to_radio, radio_status, radio_freq } =
		connect_to_radio();
