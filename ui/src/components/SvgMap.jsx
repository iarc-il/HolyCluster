...
function SvgMap({
  ...
  const rendered_spots = spots.toReversed().map((spot, index) => {
    if (spot.id == hovered_spot.id) {
      ...
      hovered_spot_distance = (
        haversine(spot.dx_loc, spot.spotter_loc) / 1000
      ).toFixed();
    }
    return (
      <Spot
      ...
      <svg
        ...
        onClick={(event) => {
          ...
          if (event.detail == 2 && distance_from_center <= radius) {
            ...
            const displayed_locator = new Maidenhead(lat, lon).locator.slice(
              0,
              6
            );
            set_map_controls(
      ...
      >
        ...
        <g className="font-medium text-lg select-none">
          <text x={text_x} y={text_y} fill={colors.theme.text}>
            Radius:{" "}
            {settings.is_miles ? km_to_miles(radius_in_km) : radius_in_km}{" "}
