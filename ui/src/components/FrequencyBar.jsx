...
export default function FrequencyBar({
	...
	function get_band_from_freq(freq) {
		for (let band of Object.keys(band_to_freq)) {
			if (freq <= band_to_freq[band][1] && freq >= band_to_freq[band][0])
				return band;
		}
	...
	let sorted_spots = useMemo(() => {
		return spots
			.filter((spot) => spot.band == radio_band)
	...
	sorted_spots = sorted_spots.reduce((acc, spot) => {
		const existingSpot = acc.find(
			(s) => s.dx_callsign === spot.dx_callsign
		);
		if (!existingSpot || existingSpot.time < spot.time) {
			return acc
				.filter((s) => s.dx_callsign !== spot.dx_callsign)
				.concat(spot);
		}
		return acc;
	}, []);
	...
	return (
		<>
			{marking_array.map((mark) => {
				return mark % 100 === 0 ? (
					<g key={`ruler_${mark}`}>
						<text
							y={`${
								((mark - min_freq) / (max_freq - min_freq)) *
								100
							}%`}
							x1={0}
							fontSize="12"
					...
					<line
						y1={`${
							((mark - min_freq) / (max_freq - min_freq)) * 100
						}%`}
						y2={`${
							((mark - min_freq) / (max_freq - min_freq)) * 100
						}%`}
			...
			<svg
				...
				width="5%"
				y={`${
					((radio_freq - min_freq) / (max_freq - min_freq)) * 100 - 4
				}%`}
				x="25%"
			>
				<polygon
