...
async def enrich_telnet_spot(qrz_session_key: str, spot: dict, debug: bool = False):
    try:
        ...
        dx_geo_cache, dx_locator_source, dx_locator, dx_lat, dx_lon, dx_country, dx_continent = await get_geo_details(
            qrz_session_key=qrz_session_key, callsign=spot["dx_callsign"], debug=debug
        )
        spot.update(
            {
        ...
        message = (
            f"**** ERROR enrich_telnet_spot **** An exception of type {type(ex).__name__} occured. Arguments: {ex.args}"
        )
        logger.error(message)
        return spot
...
async def spots_consumer(debug: bool = False):
    ...
    log_filename = "enrich_telnet_spots"
    ...
    while True:
        try:
            ...
            message = (
                f"**** ERROR spots_consumer **** An exception of type {type(ex).__name__} occured. Arguments: {ex.args}"
            )
            logger.error(message)
